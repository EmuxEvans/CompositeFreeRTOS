...
{
	r->fault_addr = fault;
	r->regs.ip = cos_thd_cntl(COS_THD_GET_FIP, tid, 1, 0);
	r->regs.sp = cos_thd_cntl(COS_THD_GET_FSP, tid, 1, 0);
	r->regs.bp = cos_thd_cntl(COS_THD_GET_FFP, tid, 1, 0);
	r->regs.ax = cos_thd_cntl(COS_THD_GET_F1,  tid, 1, 0);
	r->regs.bx = cos_thd_cntl(COS_THD_GET_F2,  tid, 1, 0);
	r->regs.cx = cos_thd_cntl(COS_THD_GET_F3,  tid, 1, 0);	
	r->regs.dx = cos_thd_cntl(COS_THD_GET_F4,  tid, 1, 0);
	r->regs.di = cos_thd_cntl(COS_THD_GET_F5,  tid, 1, 0);
	r->regs.si = cos_thd_cntl(COS_THD_GET_F6,  tid, 1, 0);
	r->tid     = tid;
	r->spdid   = spdid;
}
